function [f_est_J,w_est_J,x_est,norms,ISTA] = ISTA_flexgrid_Z_two(y_f,phi,sigma_w,L,w,f,x_f,J, co)
...
er{1,1}=er1-f(1);
er{1,2}=er2-f(2);
[C_, f_est_J] = kmeans(reshape(f_est,length(f_est),1), J);
...
norm_val = norm(x_est-x_f)/norm(x_f);
norms(1)=norm_val;
