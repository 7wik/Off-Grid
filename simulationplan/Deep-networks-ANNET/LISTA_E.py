...
class LISTA(nn.Module):
    def __init__(self, _w1_, _w2_, C, stage, alpha, thr, useGPU):
        ...
        for layer in range(stage):
            ...
            layer_params = (w_real, w_imag, b_real, b_imag, th, conv1d_1, conv1d_2)
            suffix = ''
            param_names = ['weights_real_{}{}', 'weights_imag_{}{}',
                           'bias_real_{}{}', 'bias_imag_{}{}', 'threshold{}{}', 'conv1d_1_{}{}', 'conv1d_2_{}{}']
    ...
    def forward(self, y_real, y_imag):
        ...
        for layer in range(self.stage):
            ...
            x_cnn_imag = (conv1d_2(conv1d_1(x_imag.T.view(self.bs, 1, self.n)))).view(self.bs, self.n).T
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            for i in range((nv - 1) // (2 * bs) + 1):
                start_i = i * (2 * bs)
                end_i = start_i + (2 * bs)
                y_pred_real, y_pred_imag, x_pred_real, x_pred_imag = net(yt_real[:, start_i:end_i], yt_imag[:, start_i:end_i])
