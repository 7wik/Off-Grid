...
def my_lista(A, C, Phi, y_train, x_train, y_valid, x_valid, _lambda, alpha, bs, epochs,stage, learning_rate):
    ...
    for epoch in range(epochs):
        ...
        for i in range((nt - 1) // bs + 1):
            ...
            loss = my_loss_y(C_real, C_imag, x_real, x_imag, y_gt_real[:, start_i:end_i], y_gt_imag[:, start_i:end_i])

            
            # optimizer.zero_grad() #satwik's change
            loss.backward()
            optimizer.step()
            optimizer.zero_grad()
            if i % 50 == 0:
        ...
        with torch.no_grad():
            for i in range((nv - 1) // (2 * bs) + 1):
                ...
                loss_val = my_loss_y(C_real, C_imag, x_pred_real, x_pred_imag,
